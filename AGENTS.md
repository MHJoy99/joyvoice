...
| `app/system/call_mute.py` |   —   | **Active call muter.** `CallMuteManager` + `get_call_mute_manager()`. Mutes call apps during dictation via 3 modes (`"off"`, `"hotkey"`, `"virtual_device"`). Uses `psutil` to detect running call apps (Discord, Zoom, Teams)                         |
| `app/system/mic_muter.py` |  257  | Legacy crash-recovery helper for session muting state (`recover_leftovers()`, `muted_pids.json`). Not the active live muter                                                                                                                            |
...
### Call Muting & Session Protection (opt-in, `mute_other_apps`)

Call muting in JoyVoice is actively managed by `app/system/call_mute.py` (`CallMuteManager` + `get_call_mute_manager()`). Configured in Settings (Audio tab), `mute_other_apps` stores a mode string (`"off"`, `"hotkey"`, or `"virtual_device"`), where legacy boolean `true` maps to `"hotkey"` and `false` to `"off"`.
...
- **UI & Feedback**:
  ...
  - Runtime feedback (`app/main.py` `_notify_mute_status`) surfaces warnings via floating widget toasts when no call apps are detected, no virtual device is configured, or keybind guidance is needed.
- **Legacy Crash Recovery (`mic_muter.py`)**:
  - `app/system/mic_muter.py` is no longer the active live muter. It only provides startup crash recovery (`recover_leftovers()` reading `%APPDATA%\JoyVoice\muted_pids.json`) to un-mute any leftover sessions from prior abnormal shutdowns.
- **PyInstaller EXE Support**:
  - PyInstaller spec files include `pycaw`, `comtypes`, and `psutil` in `hiddenimports` so call muting functions seamlessly in frozen builds.
...
2. **Verify** with the project tests, isolated import checks, `git diff --check`, and `python bin/guard.py pre-commit`. Confirm `.env`, credentials, EXEs, `dist`, and release artifacts are not staged. Update every version surface (`pyproject.toml`, `schema.json`, `README.md`, `CHANGELOG.md`, `AGENTS.md`, and `AI_STATUS.md`). Perform discovery/SEO verification before tagging: verify `llms.txt`, `llms-full.txt`, `schema.json`, `index.html`, `README.md`, `robots.txt`, repository topics, and GitHub homepage/description are current and reference canonical URLs and the new version. Then inspect `git status`, `git diff`, and `git log`.
